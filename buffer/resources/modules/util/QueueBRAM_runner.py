...
class QueueBRAM_runner(ModuleRunner):
    
    def __init__(self, max_runners=1000):
        super().__init__("QueueBRAM", max_runners)
        self.parameters = {
            "data_width": 0,
            "binary_point": 0,
            "entries": 0,
    ...
    def gen_parameters(self):
        ...
        self.parameters["entries"] = random.randint(1, 12288) #
    ...
    #Dict keys order must corresponds with Chisel Module input parameters
        ...
        inst = "fpgaconvnet.util.{module}({params})".format(
            module=self.name,
            params=params
            )
