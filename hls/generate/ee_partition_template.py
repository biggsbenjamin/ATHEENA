ee_network_header_template = """#ifndef {NAME}_TOP_HPP_
...
ee_network_src_template = """#include "{name}_top.hpp"
...
void process(
    ...
    //volatile mem_int in_hw[{NAME}_PORTS_IN][{NAME}_SIZE_IN],
    //volatile mem_int out_hw[{NAME}_PORTS_OUT][{NAME}_SIZE_OUT]
    //something like the layer test inputs
    {io_stream}
)
{{
...
{layers}
    ...
    //mem_write< //NOTE not using these in profiling mode
    //    {NAME}_BATCH_SIZE,
    //    {NAME}_ROWS_OUT,
    //    {NAME}_COLS_OUT,
    //    {NAME}_CHANNELS_OUT,
...
void fpgaconvnet_ip(
    ...
    //volatile mem_int in_hw[{NAME}_PORTS_IN][{NAME}_SIZE_IN],
    //volatile mem_int out_hw[{NAME}_PORTS_OUT][{NAME}_SIZE_OUT]
    //replace with the same inputs as above
    {io_stream}
)
{{
...
#pragma HLS INTERFACE m_axi port=in  offset=slave depth=size_in  num_read_outstanding=1 num_write_outstanding=1 max_read_burst_length=256 max_write_burst_length=256 name=fpgaconvnet_in  bundle=fpgaconvnet_port_in

#pragma HLS INTERFACE m_axi port=out offset=slave depth=size_out num_read_outstanding=1 num_write_outstanding=1 max_read_burst_length=256 max_write_burst_length=256 name=fpgaconvnet_out bundle=fpgaconvnet_port_out


    #pragma HLS DATAFLOW
    if ( mode == 0 ) {{
        //need to change this to match ealier input args
        process(weights_reloading_index,in,out);
    }} else if ( mode == 1 ) {{
#if {NAME}_WEIGHTS_RELOADING_FLAG
...
ee_network_tb_src_template = """#include "{name}_top.hpp"
#include "common_tb.hpp"

int main()
{{
    int err = 0;
    std::string input_path  = "{data_path}/input.dat";
    std::string output_path = "{data_path}/output.dat";
    ...
    //change type and array dimensions - might need diff ones for diff layers...

    stream_t(data_t) in[{NAME}_STREAMS_IN];

    static data_t test_in[{NAME}_SIZE_IN][{NAME}_STREAMS_IN];
    ...
    for( int wr_index=0;wr_index<{NAME}_WEIGHTS_RELOADING_FACTOR;wr_index++) {{
        ...
        stream_t(data_t) out[{NAME}_STREAMS_OUT];
        stream_t(data_t) out_valid[{NAME}_STREAMS_OUT];
        static data_t test_out[{NAME}_SIZE_OUT][{NAME}_STREAMS_OUT];
        ...
        ////>(output_path,"out",test_out_valid,wr_index);

        to_stream<
            {NAME}_SIZE_IN,
            {NAME}_STREAMS_IN
        >(test_in, in);
        std::cout<<"STREAM OUT VALID"<<std::endl;
        std::cout<<{NAME}_SIZE_OUT<<std::endl;
        std::cout<<{NAME}_STREAMS_OUT<<std::endl;
        to_stream<
            {NAME}_SIZE_OUT,
            {NAME}_STREAMS_OUT
        >(test_out, out_valid);
...
#if {NAME}_WEIGHTS_RELOADING_FLAG
        if( wr_index > 0 ) {{
            fpgaconvnet_ip(1,wr_index,weights,in,out);
        }}
        fpgaconvnet_ip(0,wr_index,weights,in,out);
#else
        fpgaconvnet_ip(0,wr_index,in,out);
#endif
        //for(int i=0; i<{NAME}_PORTS_OUT;i++) {{
        for(int i=0; i<{NAME}_STREAMS_OUT;i++) {{
